...
import { getFriends, getHero, getHuman, getDroid, getReview, saveReview } from './starWarsData';
...
const reviewEntryType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    commentary: {
      type: GraphQLString,
      description: 'is this a great movie?'
    }
  })
});
...
const queryType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    /* kumokay: add review related query */
    review: {
      type: GraphQLList(reviewEntryType),
      args: {
        episode: {
          description:
            'If omitted, returns all reviews. If ' +
            'provided, returns reviews of that particular episode.',
          type: episodeEnum
        },
        id: {
          description:
            'If omitted, returns all reviews. If ' +
            'provided, returns the review of that particular id.',
          type: GraphQLString
        },
      },
      resolve: (root, { episode, id }) => getReview(episode, id)
    }
  }),
});


/* kumokay: add review related mutation */
const reviewInputType = new GraphQLInputObjectType({
  name: 'Review',
  description: 'Review for an episode',
  fields: () => ({
    stars: {
      type: GraphQLInt
    },
...
const mutationType = new GraphQLObjectType({
  ...
  fields: () => ({
    createReview: {
      ...
      resolve: (root, { episode, review }) => saveReview(episode, review)
    }
  })
});
